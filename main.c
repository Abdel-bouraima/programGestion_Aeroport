...
/* A REVOIR 
Salire: trouver le nombre d'heure de travail* taux horaire ou salaire de base * nombre de vol par mois (comme les journalistes)
si par exemple un pilote n'est pas trouvé, doit-on lui redemander à chaque fois son id (while) ou utiliser un if ???
la saisie : problème avec le fgets. 
Place première classe, classe économique etc.
Le nombre de place en première doit-il est fixé lors de l'enregistrement de l'avion ou lors de celui du vol?
...
ajout_avion
Ajout vol: permet d'ajouter des vols
Modification vol: modifier les éléments
Supprimer/annuler vol: annuler un vol*/

void ajout_avion (int *ne, avion t[], int identifiant, string designation, string modele, int place_max, int nombre_utilisation, string date_achat, float prix_achat)
{
    t[*ne].identifiant_avion = identifiant;
    strcpy(t[*ne].designation_avion, designation);
    strcpy(t[*ne].modele_avion, modele);
    t[*ne].capacite = place_max;
...
void affiche_tab_vol (int n, vol t[])
{
    ...
    while (i < n)
    {
        ...
        printf("          Matricule du co-pilote: %d\n", t[i].matricule_pilote2);
...
int recherche_pilote_par_id (int n, pilote t[], int identifiant)
{
    int i;
    int flag;
    ...
    do
    {
        ...
        if(choix_utilisateur == 1)
        {
            ...
            do
            {
                ...
                printf("12: Quitter l'espace admin\n");
                ...
                if(choix_admin == 1)
                {
                    ...
                    else
                    {
                        ...
                        if (recherche_avion (nbre_avion, tab_avion, id_plane) == 0)
                        {
                            ...
                            printf("Entrez la capacité maximale de l'avion\n");
                            scanf("%d", &capacity_plane);
                            fflush(stdin);
                            ...
                            ajout_avion (&nbre_avion, tab_avion, id_plane, designation_plane, model_plane, capacity_plane, number_fly, date_buy_plane, price_buy_place);
                            printf("Ajout réussi \n");
                        }
                    }
                }
                                                            ...
                                                            if (choix_2 == 3)
                                                            {
                                                                ...
                                                                if (recherche_pilote_par_id (nbre_pilote, tab_pilote, id_pilote) == 1)
                                                                {
                                                                    ...
                                                                    if (recherche_pilote_par_id (nbre_pilote, tab_pilote, id_copilote) == 1)
                                                                    {
                                                                        ...
                                                                        {
                                                                            // Lancer le programme de modification
                                                                            modifie_id_pilote_vol (nbre_vol, tab_vol, id_pilote, id_copilote);
                                                                            printf("Modification réussie \n");
                                                                        }
                                                                        else
                                                                        {
